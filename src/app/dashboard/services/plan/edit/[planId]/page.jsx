...
export default function EditPlanPage() {
  ...
  useEffect(() => {
    const fetchPlan = async () => {
      try {
        ...
        const plan = all.find((p) => p._id === planId);
        ...
        setForm({
          ...
          serviceId: plan.serviceId?._id || plan.serviceId,
        });
      } catch {
  ...
  const savePlan = async () => {
    if (!form.name || !form.durationDays) {
      toast.error('Please fill all required fields');
      return;
    }
    setLoading(true);
    try {
      await apiClient.patch(`/api/services/plans/${planId}`, {
        name: form.name,
        price: form.billing_cycle === 4 ? 0 : form.price,
  ...
  if (!form) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="w-12 h-12 border-4 border-[#FF5211] border-t-transparent rounded-full animate-spin"></div>
      </div>
    );
  }
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">Update plan details</p>
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="group">
                ...
                <input
                  ...
                  value={form.price}
                  onChange={handleChange}
                  disabled={Number(form.billing_cycle) === 4}
                ...
                />
                {Number(form.billing_cycle) === 4 && (
                  <p className="text-xs text-gray-500 mt-2 flex items-center gap-1">
                    <CheckCircle2 className="w-3 h-3" /> Trial plans are free
                  </p>
                )}
              </div>
                ...
                {billingCycleOptions.map((option) => (
                  <button
                    ...
                    className={`relative p-4 rounded-xl border-2 transition-all duration-300 ${
                      Number(form.billing_cycle) === option.value
