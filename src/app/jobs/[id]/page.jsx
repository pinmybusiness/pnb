...
import {
  ...
  Building, Briefcase, ArrowLeft,
  FileText, CheckCircle, Languages,
  UserCheck, Gift, Utensils, Home,
  Users
} from "lucide-react";
import Header from "@/components/Header";
import Link from "next/link";
import { formatDateWithSuffix } from "@/utils/dateFormat";
import { getCategoryText, getDayName, getInternshipTypeText, getShiftText, getStipendText } from "@/utils/opportunity";
...
export default function PublicOpportunityDetail() {
  ...
  const handleApply = () => {
    ...
    router.push(`/applications/apply/${id}`);
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <ul className="space-y-2 text-sm text-gray-700">
              ...
              {opportunity.schedule?.endDate && (
                <li className="flex justify-between">
                  <span>End Date:</span>
                  <span className="font-medium">
                    {formatDateWithSuffix(opportunity.schedule.endDate)}
                  </span>
                </li>
              )}
              ...
              {opportunity.schedule?.days && opportunity.schedule.days.length > 0 && (
                <li>
                  <div className="flex justify-between mb-1">
                    ...
                    <span className="font-medium">
                      {opportunity.schedule.days.length} days/week
                    </span>
                  </div>
              ...
              {opportunity.schedule?.shift && (
                <li className="flex justify-between">
                  <span>Shift:</span>
                  <span className="font-medium">
                    {getShiftText(opportunity.schedule.shift)}
                  </span>
                </li>
              )}
              {opportunity.schedule?.hoursPerDay && (
                <li className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {opportunity.schedule.hoursPerDay} hours
                  </span>
                </li>
              )}
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            ...
            <ul className="space-y-2 text-sm text-gray-700">
              ...
              {opportunity.filledPositions > 0 && (
                <li className="flex justify-between">
                  <span>Filled Positions:</span>
                  <span className="font-medium">
                    {opportunity.filledPositions}
                  </span>
                </li>
              )}
              {opportunity.stipend?.paymentType && (
                <li className="flex justify-between">
                  ...
                  <span className="font-medium capitalize">
                    {opportunity.stipend.paymentType.replace(/_/g, " ")}
                  </span>
                </li>
              )}
