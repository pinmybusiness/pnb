...
const WEBSITES = [
  {
    id: 1,
    label: "PrideLocation.com",
    brand: "PrideLocation",
    baseUrl: "https://www.pridelocation.com",
    videoChatUrl: "https://www.pridelocation.com",
    apiWebsiteParam: 1,
  },
];
...
export default function PromptBuilder() {
  ...
  const [brandName, setBrandName] = useState("PrideLocation.com");
  ...
  // Persona
    const [personaName, setPersonaName] = useState("LGBTQ+ individual looking for safe online connections");
    const [painPoints, setPainPoints] = useState([
    "Experiencing discrimination on mainstream chat platforms",
    "Lack of identity-safe spaces",
    "Fear of harassment or trolling",
    ]);
    const [goals, setGoals] = useState([
    "Meet like-minded LGBTQ+ people",
    "Connect in a safe and respectful environment",
    "Express identity freely without judgment",
    ]);
...
Topic: "${topic}"
Website Context: ${selectedWebsite?.brand || "PrideLocation.com"}
Page Type: ${pageType}
...
Focus on LGBTQ+, gay, bisexual, transgender, and queer-friendly random video chat platforms.
Emphasize inclusivity, identity-safe spaces, moderation, privacy, and respectful connections.
Avoid adult framing. Keep tone community-safe and affirming.`.trim();
  ...
  const generateContentPrompt = () => {
    ...
    const payload = {
      role: "system",
        instruction: `You are an expert LGBTQ+ community content strategist and SEO Head.

        Your job is to create inclusive, respectful, and highly optimized content for PrideLocation, a Pride-focused random video chat platform.

        The content must:
        - Follow Google Search Essentials and E-E-A-T
        - Be inclusive, affirming, and culturally aware
        - Emphasize safety, moderation, identity-respect, and privacy
        - Avoid adult or explicit positioning
        - Be helpful for LGBTQ+ individuals seeking safe online connections

        Optimize for both search engines and AI Overviews.
        Content must feel human, trustworthy, and community-focused.`,
      inputs: {
  ...
  const generateResearchData = async () => {
    ...
    try {
      const response = await fetch(RESEARCH_API, {
        ...
        body: JSON.stringify({
          topic: topic,
          websiteContext: selectedWebsite?.brand || "PrideLocation.com",
          pageType: pageType,
  ...
  const autoFillInternalLinks = async () => {
    ...
    try {
      // Video chat link entry (fixed)
        ...
        preferred_anchors: [
            "start LGBTQ+ video chat",
            "join pride random chat",
            "connect with LGBTQ+ community online",
        ],
        context_hint: "LGBTQ+ safe video chat platform conversion link",
        };
      ...
      const articleEntries = await Promise.all(
        latestTwo.map(async (post) => {
          ...
          return {
            ...
            "explore pride-friendly chat tips",
            "LGBTQ+ community guide",
            ],
