...
export default function CandidateProfileForm({
  ...
  const [showAddress, setShowAddress] = useState(!!initialProfile?.address?.city);
  const [showExperience, setShowExperience] = useState(!!initialProfile?.experience?.length);
  const [showPreferences, setShowPreferences] = useState(
    !!initialProfile?.preferences?.preferredLocations?.length ||
    !!initialProfile?.preferences?.preferredCategories?.length ||
    !!initialProfile?.preferences?.preferredOpportunityTypes?.length
  );
    ...
    <div className={`min-h-screen bg-gradient-to-b from-${theme.backgroundGradientFrom} to-${theme.backgroundGradientTo} py-8 px-4`}>
      <div className="max-w-3xl mx-auto bg-white rounded-lg shadow-lg p-6">
        <h1 className={`text-2xl font-semibold text-${theme.textPrimary} mb-2 flex items-center`}>
          <Utensils className={`mr-2 text-${theme.textSecondary}`} size={24} /> Complete Your FasterQ Profile
        </h1>
